...
# All csvs were imported in a single xlsx file
...
Size_list = len(model["REF_export"]["LDF_baseline_q"])
first_ID = 4
all_difference_sheets = []
for ii in range(len(sheet_names[first_ID:])):
	all_difference_sheets.append(difference(model[sheet_names[first_ID+ii]]["LDF_baseline_q"],model["REF_export"]["LDF_baseline_q"]))
...
print('sum', np.sum(Si)+ np.sum(Sij))

print('sum', np.sum([100*x for x in Si])+ np.sum([100 * x for x in Sij]))
